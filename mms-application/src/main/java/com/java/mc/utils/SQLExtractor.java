...
//		}
	...
	private String eliminateLastColon(String sql) {
		sql = sql.trim();
		if (org.springframework.util.StringUtils.endsWithIgnoreCase(sql, ";")) {
	...
	private int lastDelimiterPosition(StringBuffer buf, String currentLine) {
		if (strictDelimiterMatching) {
			...
			if (delimiterType.equals(DelimiterType.NORMAL)) {
				...
				while (endIndex >= 0) {
					if (buf.substring(bufferIndex, bufferIndex + 1).toLowerCase(Locale.ENGLISH).charAt(0) != d
							.charAt(endIndex)) {
						return -1;
					}
					bufferIndex--;
				...
				return currentLine.trim().toLowerCase(Locale.ENGLISH).equals(d) ? buf.length() - currentLine.length()
						: -1;
			}
		}
	}
